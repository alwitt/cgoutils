...
func (c *engineImpl) PBKDF(
	...
	if len(passwd) < C.crypto_pwhash_PASSWD_MIN || len(passwd) > C.crypto_pwhash_PASSWD_MAX {
		...
		log.
			...
			Error("PBKDF Key derivation input failure")
	...
	if saltLen, err := salt.GetLen(); err != nil {
		log.
			WithError(err).
			WithFields(goutils.UpdateCodePositionInTags(logTags)).
			Error("Unable to read PBKDF salt length")
	...
	if err != nil {
		log.
			WithError(err).
			WithFields(goutils.UpdateCodePositionInTags(logTags)).
			Error("Unable to get output buffer pointer")
	...
	if err != nil {
		log.
			WithError(err).
			WithFields(goutils.UpdateCodePositionInTags(logTags)).
			Error("Unable to get salt pointer")
